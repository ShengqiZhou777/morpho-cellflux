...
## How it's used
- `configs/perturbmulti_*.yaml` here are runtime configs; the authoritative copies live in
  `../configs/cellflux_external/` and are copied in by `scripts/launch_cellflux_pm.sh` /
  `scripts/interpolate_cellflux.sh` (which set `CELLFLUX_DIR=$PROJECT_DIR/cellflux`).
- Data paths inside those YAMLs point at `../data/processed/cellflux_ext/` (this repo).

The old external clone at `/home/ubuntu/data/sqzhou/projects/CellFlux` is now DEPRECATED —
do not edit it; this vendored copy is the source of truth.
