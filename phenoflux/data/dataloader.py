...
def _load_pairing_json(args):
    """Load precomputed pairing JSON if merfish_nn mode is set."""
    import json
    pmode = getattr(args, "pairing_mode", "batch_random")
    ppath = getattr(args, "pairing_path", None)
    if pmode == "merfish_nn" and ppath is not None:
        with open(ppath) as f:
            data = json.load(f)
        d = data.get("pairing", data)
        print(f"Loaded {len(d)} precomputed pairings from {ppath}")
        return d
    return None


def _load_cluster_map(args):
    """Load cluster_type assignments if cluster_match mode is set."""
    import pandas as pd
    pmode = getattr(args, "pairing_mode", "batch_random")
    ppath = getattr(args, "pairing_path", None)
    if pmode == "cluster_match":
        csv_path = ppath or args.data_index_path
        df = pd.read_csv(csv_path, index_col=0)
        m = dict(zip(df["SAMPLE_KEY"], df["cluster_type"]))
        print(f"Loaded {len(m)} cluster assignments from {csv_path}")
        return m
    return None
    ...
    def __init__(self, args, device):
        ...
        self.fold_datasets = {
            'train': CellDatasetFold('train',
                                     ...
                                     pairing_mode=getattr(args, "pairing_mode", "batch_random"),
                                     precomputed_pairing=_load_pairing_json(args),
                                     cluster_map=_load_cluster_map(args),
                                     augment_strength=getattr(args, "augment_strength", "default")),
...
class CellDatasetFold(Dataset):
    ...
    def __init__(self,
                 ...
                 pairing_mode='batch_random',
                 precomputed_pairing=None,
                 cluster_map=None,
                 augment_strength='default'):
        """
        ...
        self.precomputed_pairing = precomputed_pairing
