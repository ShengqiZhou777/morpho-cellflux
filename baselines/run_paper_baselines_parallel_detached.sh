...
setsid nohup bash -c '
set -uo pipefail
PROJECT="$1"; LOG="$2"; CONDA_BIN="$3"; CONDA_ENV="$4"
cd "$PROJECT"
{
  echo "[$(date -Is)] START detached parallel paper baseline queue"
  echo "project=$PROJECT"
  "$CONDA_BIN" run --no-capture-output -n "$CONDA_ENV" bash baselines/run_paper_baselines_parallel.sh
  code=$?
  echo "[$(date -Is)] DONE detached parallel paper baseline queue exit=$code"
  exit "$code"
} >> "$LOG" 2>&1
' _ "$PROJECT" "$LOG" "$CONDA_BIN" "$CONDA_ENV" >/dev/null 2>&1 &
